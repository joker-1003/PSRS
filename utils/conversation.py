...
class Conversation:
    ...
    def get_prompt(self):
        """Generate the prompt based on the separator style."""
        if self.sep_style == SeparatorStyle.QWEN:
            ret = ""
            if self.system:  # Optional system message
                ret += f"<|im_start|>system\n{self.system}<|im_end|>\n"
            for i, (role, message) in enumerate(self.messages):
                ret += f"<|im_start|>{role}\n{message}<|im_end|>"
                if i < len(self.messages) - 1 or role == "user":
                    ret += "\n"
...
# Register all conversation templates
conv_templates = {
    "baize": conv_baize,
...
def get_default_conv_template(model_name):
    """Return the default conversation template based on the model name."""
...
if __name__ == "__main__":
    conv = conv_templates["qwen_vl"].copy()
    conv.append_message("user", "<|vision_start|><|image_pad|><|vision_end|>What is in this image?")
